...
def _openmm_minimize(
    ...
    """
        current situation: trying to add ligands to the simulation this seems to be possible to by first creating RDKit
        mol then getting the PDB string and then using model to merge the two molecules.
        See the following example code:  https://github.com/choderalab/openmoltools/blob/master/examples/test_protein_ligand.py
        However, we also require to modify the force field to be able to reason about the ligand as well.
        Raised issues https://github.com/choderalab/openmoltools/issues/309 and https://github.com/openmm/openmm/issues/160
