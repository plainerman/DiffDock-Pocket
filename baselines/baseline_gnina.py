...
parser.add_argument('--seed_molecule_includes_name', action='store_true', default=False, help='additionally add the complex name at the beginning of the seedmolecule path')
...
parser.add_argument('--gnina_path', type=str, default='', help='Path to the gnina executable')
...
if args.use_p2rank_pocket and not args.skip_p2rank:
    ...
    with open(f"{p2rank_cache_path}/pdb_list_p2rank.txt", "w") as out:
        for name in names:
            if os.path.exists(os.path.join(args.data_dir, name, f'{name}{args.protein_file_suffix}.pdb')):
                shutil.copy(os.path.join(args.data_dir, name, f'{name}{args.protein_file_suffix}.pdb'), f'{pdb_files_cache}/{name}{args.protein_file_suffix}.pdb')
                out.write(os.path.join('pdb_files', f'{name}{args.protein_file_suffix}.pdb\n'))
...
for i, name in enumerate(names):
    ...
    rec_path = os.path.join(args.data_dir, name, f'{name}{args.protein_file_suffix}.pdb')
    prediction_output_name = os.path.join(args.results_path, name, f'{name}{args.file_suffix}.pdb')
    flexRes_output_name = os.path.join(args.results_path, name, f'{name}_flex_residues.pdb')
    log_path = os.path.join(args.results_path, name, f'{name}{args.file_suffix}.log')
    if args.seed_molecules_path is not None: seed_mol_path = os.path.join(args.seed_molecules_path, name, f'{name if args.seed_molecule_includes_name else ""}{args.seed_molecule_filename}')
    ...
    if args.use_p2rank_pocket:
        if os.path.exists(f'{p2rank_cache_path}/p2rank_output/{name}{args.protein_file_suffix}.pdb_predictions.csv'):
            df = pd.read_csv(f'{p2rank_cache_path}/p2rank_output/{name}{args.protein_file_suffix}.pdb_predictions.csv')
            ...
            if df.empty:
                ...
                if args.flexdist > 0:
                    ...
                    conf.SetAtomPosition(0, Point3D(center_x, center_y, center_z))
                    pocket_center_cache_path = f'{p2rank_cache_path}/p2rank_output/{name}_pocketcenter_as_carbon.pdb'
                    MolToPDBFile(mol, pocket_center_cache_path)
        else:
            fallback_without_p2rank = True


    # create flexibility arguments
    if args.flexdist > 0 and args.seed_molecules_path is not None and not args.use_p2rank_pocket: # for EquiBind plus GNINA flexible and GNINA rigid plus GNINA flexible
        flex_args = f'--flexdist {args.flexdist} --flexdist_ligand {seed_mol_path} {"--flex_max " + str(args.flex_max) if args.flex_max > 0 else ""} --out_flex {flexRes_output_name}'
    elif args.flexdist > 0 and args.seed_molecules_path is None and not args.use_p2rank_pocket: # for GNINA flexible (all sidechains flexible with high enough flexdist)
        flex_args = f'--flexdist {args.flexdist} --flexdist_ligand {rdkit_mol_path} {"--flex_max " + str(args.flex_max) if args.flex_max > 0 else ""} --out_flex {flexRes_output_name}' 
    elif args.flexdist > 0 and args.seed_molecules_path is None and args.use_p2rank_pocket and not fallback_without_p2rank: # for GNINA flexible (all sidechains flexible with high enough flexdist)
        flex_args = f'--flexdist {args.flexdist + diameter_pocket/2} --flexdist_ligand {pocket_center_cache_path} {"--flex_max " + str(args.flex_max) if args.flex_max > 0 else ""} --out_flex {flexRes_output_name}'
    else:
        flex_args = ''
